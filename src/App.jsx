...
export default function ChingazaApp() {
  ...
  async function handleDeleteReport(reporte) {
    const ok = window.confirm("¿Borrar este reporte? No se puede recuperar.");
    if (!ok) return;
    setReporteError("");
    try {
  ...
  const listaReportes = verMisReportes ? misReportesVisibles : reports.slice(0, 4);
