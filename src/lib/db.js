...
export async function updateBolsasRemote(rowId, bolsas) {
  if (!supabaseEnabled) return;
  const { error } = await supabase.from("jornada_desglose").update({ bolsas }).eq("id", rowId);
  if (error) throw error;
}

export async function updateDestinoRemote(rowId, destino) {
  if (!supabaseEnabled) return;
  const { error } = await supabase.from("jornada_desglose").update({ destino }).eq("id", rowId);
  if (error) throw error;
}

export async function updateKgRemote(jornadaId, kgTotal) {
  if (!supabaseEnabled) return;
  const { error } = await supabase.from("jornadas").update({ kg_total: kgTotal }).eq("id", jornadaId);
  if (error) throw error;
}

export async function updateParticipantesRemote(jornadaId, participantes) {
  if (!supabaseEnabled) return;
  const { error } = await supabase.from("jornadas").update({ participantes }).eq("id", jornadaId);
  if (error) throw error;
}
